...
Ajouts découverte À TESTER EN LIVE (comblent les 2 plus gros écarts cours/défi) : decouverte-step-ca étape 4 « Émettre un certificat SSH » (init niveau 0 passé en `--ssh`, étape ancienne « Inspecter » devenue step5) ; decouverte-openbao étape 4 « policy + AppRole » (étape ancienne « Révoquer » devenue step5). Bannières « À TESTER » en tête de ces deux étapes ; commandes SSH/AppRole non encore validées sur Killercoda.
