...
decouverte-step-ca-token étape 4 « ACME : le renouvellement qui se déclenche tout seul » (provisioner ACME + acme.sh HTTP-01 standalone, /etc/hosts acme.lab.local→127.0.0.1, CURL_CA_BUNDLE=root_ca.crt ; étape ancienne « Automatiser » devenue step5) : AJOUTÉE, À TESTER EN LIVE (bannière en place). Points fragiles : `step ca provisioner add acme --type ACME` + restart, URL directory `/acme/acme/directory`, acme.sh standalone sur :80 + socat, confiance TLS via CURL_CA_BUNDLE (fallback `--insecure`).
